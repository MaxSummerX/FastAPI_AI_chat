...
- **JWT авторизация** — access/refresh токены, bcrypt, **система инвайтов** для регистрации
...
- **hh.ru** — веб-парсер вакансий (скрейпинг HTML-страниц, без публичного API) с актуализацией статуса
...
- **Слоистая архитектура** (в духе Clean Architecture): `domain/` <- `application/` <- `infrastructure/` <- `presentation/`
- **Service Layer** — 10+ сервисов в `app/application/services/`
...
|----------|---------------------------------------------------|
| **Backend** | Python 3.13+, FastAPI 0.120+, Uvicorn, Pydantic 2.12+ |
| **Database** | PostgreSQL, Qdrant, Neo4j, Redis |
| **ORM** | SQLAlchemy 2.0 async, Alembic |
| **Memory/AI** | mem0ai, OpenAI/OpenRouter, Ollama, langchain-neo4j |
| **Search** | TSVECTOR (PostgreSQL FTS), rank-bm25 |
| **Web** | ddgs (DuckDuckGo), readability-lxml, beautifulsoup4 |
...
- Clean Architecture и Service Layer
