...
async def is_conversation_empty_async(conversation: dict[str, Any]) -> bool:
    """
    Асинхронно проверяет, является ли диалог пустым

    Args:
        conversation: объект диалога

    Returns:
        bool: True если диалог пустой, False в противном случае
    """
    # Эта функция не содержит I/O операций, но оставлена асинхронной
    # для консистентности API и будущих расширений
    ...
    for message in chat_messages:
        ...
        if text:  # Если есть непустой текст
            return False

        # Также проверяем content на наличие текста
...
async def process_conversations_stream(
    ...
    try:
        ...
        for conversation in conversations:
            ...
            # Проверяем, пустой ли диалог
            if skip_empty and await is_conversation_empty_async(conversation):
                skipped_empty += 1
                logger.info(f"Пропущен пустой диалог: {uuid}")
                continue
